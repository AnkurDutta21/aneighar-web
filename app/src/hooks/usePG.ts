...
import type { ImageAsset } from '@/api/pg';

/**
 * PG query key factory — identical to web hooks/usePG.ts.
 */
export const pgKeys = {
  all: ['pg'] as const,
...
export function useUploadImages() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: ({ id, assets }: { id: string; assets: ImageAsset[] }) =>
      pgApi.uploadImages(id, assets),
    onSuccess: (_, { id }) => {
      qc.invalidateQueries({ queryKey: pgKeys.detail(id) });
    },
  });
